...
dt.int <-
  ...
  distinct() %>% 
  pivot_wider(id_cols = c(sample_id, tissueID), 
...
wellsPerInt(int.spn2[is.na(outlier)])
...
mod_gam1 = gam(ocr~s(t)+gt+age+gt*age)
summary(mod_gam1)
mod_gam2 = gam(ocr~s(t,by = gt)+gt+age+gt*age)
summary(mod_gam2)
...
for(age in unique(dt.norm.means.ocr$AgeGrp)){
  ...
  mod_gam1 = gam(ocr~s(t)+gt)
  mod_gam2 = gam(ocr~s(t,by = gt)+gt)
